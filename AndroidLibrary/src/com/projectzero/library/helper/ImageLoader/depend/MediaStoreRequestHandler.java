...
import static com.projectzero.library.helper.ImageLoader.depend.Picasso.LoadedFrom.DISK;

class MediaStoreRequestHandler extends com.projectzero.library.helper.ImageLoader.depend.ContentStreamRequestHandler {
  ...
  @Override public Result load(Request request, int networkPolicy) throws IOException {
    ...
    if (request.hasSize()) {
      ...
      if (!isVideo && picassoKind == MediaStoreRequestHandler.PicassoKind.FULL) {
      ...
      if (isVideo) {
        ...
        int kind = (picassoKind == MediaStoreRequestHandler.PicassoKind.FULL) ? Video.Thumbnails.MINI_KIND : picassoKind.androidKind;
  ...
  static PicassoKind getPicassoKind(int targetWidth, int targetHeight) {
    if (targetWidth <= MediaStoreRequestHandler.PicassoKind.MICRO.width && targetHeight <= MediaStoreRequestHandler.PicassoKind.MICRO.height) {
      return MediaStoreRequestHandler.PicassoKind.MICRO;
    } else if (targetWidth <= MediaStoreRequestHandler.PicassoKind.MINI.width && targetHeight <= MediaStoreRequestHandler.PicassoKind.MINI.height) {
